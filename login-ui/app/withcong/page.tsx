...
const Login = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Simulated last login date (replace with real data from your backend)
    const lastLoginDate = new Date("2024-06-01"); // for example
    const now = new Date();
    const diffInDays = Math.floor((now.getTime() - lastLoginDate.getTime()) / (1000 * 60 * 60 * 24));

    if (diffInDays > 10) {
      setErrorMsg("Last connection superior to 10 days");

      // Redirect to verification page after 2-3 seconds
      setTimeout(() => {
        ...
        <div className="w-full h-full md:w-1/2 bg-[#c2d5d4] flex items-center justify-center">
          ...
          >
            ...
            {/* Google Login */}
              <button
                type="button"
                onClick={() => signIn("google", { callbackUrl: "/App_page" })}
            ...
            {/* Email */}
            <label htmlFor="email" className="text-sm text-[#0b0b0b] font-medium mb-1 block">
              Email
            </label>
            <input
              type="email"
            ...
            {/* Password */}
            <label htmlFor="password" className="text-sm text-[#0b0b0b] font-medium mb-1 block">
              Password
            </label>
            <input
              type="password"
